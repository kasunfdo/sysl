...
func applyTranformToModel(modelName, transformAppName, viewName string, model, transform *sysl.Module) *sysl.Value {
	...
	if len(view.Param) >= 2 {
		...
		for _, tName := range tNames {
			...
			eval.AppendItemToValueList(result.GetList(), eval.EvalView(transform, transformAppName, viewName, s))
		}
	} else {
		result = eval.EvalView(transform, transformAppName, viewName, s)
	}
